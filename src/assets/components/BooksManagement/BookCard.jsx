  ...
  return (
    <Link to={`/book-details/${book?._id}`}  className="w-full max-w-[130px] xs:max-w-[270px] sm:max-w-[270px] bg-white dark:bg-gray-900 rounded-md shadow-md overflow-hidden transform transition hover:-translate-y-1 hover:shadow-xl">
      ...
      <div className="p-2 sm:p-3">
        <h3 className="text-[9px] sm:text-lg font-semibold text-black dark:text-white line-clamp-2 h-7 sm:h-13">
          {book?.title}
        </h3>

        <p className="sm:mt-1 text-[8px] sm:text-sm text-gray-500">{book?.author}</p>
        ...
        <div className="mt-1 sm:mt-3 flex items-center justify-between">
          ...
          <div className="text-[8px] sm:text-sm font-medium text-green-700">{book?.genre}</div>
        </div>
